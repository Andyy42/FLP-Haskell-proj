...
doAllExperiments exp = do
    let lossFun = expLossFunction exp; seed = expSeed exp
        arch = expArchitecture exp; paths = expDataPaths exp
        batchSize = expBatchSize exp; epochs = expEpochs exp
        lr = expLearningRate exp; name = expName exp
