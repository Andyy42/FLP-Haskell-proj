...
import           NeuralNetwork         (backward, batchedTrainLoop,
                                        createBatches, evaluateLoss,
                                        evaluateLossBatched, forward,
                                        gradientDescent, newB, newW,
...
import           Types                 (Activation (..), BackpropagationStore,
                                        DataPaths (..), DeltasMatrix,
                                        Experiment (..), Gradients, InMatrix,
...
createLinearLayer :: LinearLayerConfig  -- ^ The configuration for the linear layer
    ...
    [filename] -> do
      ...
      case parseExperiment input of
        ...
        Right newExperiment -> let exp = newExperiment in do
          ...
          datas@(inputs, targets) <- loadBatched (expDataPaths exp) (expBatchSize exp)
          -- Init Neural Network
          putStrLn "Init NN"
          let nn = createNN (expArchitecture exp) seed
          print nn
          putStrLn "Batch train"
          let (lossValue, trainedNN) = batchedTrainLoop (expEpochs exp) nn lossFun inputs targets (expLearningRate exp) seed
          in2 <- loadMatrix $ dpInput (expDataPaths exp)
          tgt2 <- loadMatrix $ dpTarget (expDataPaths exp)
          let (lossValue2, trainedNN2) = trainLoop (expEpochs exp) nn lossFun in2 tgt2 (expLearningRate exp) 

          putStrLn $ "Loss2 after training: " ++  show (evaluateLoss trainedNN2 lossFun (in2,tgt2))
          putStrLn $ "Loss before training: " ++  show (evaluateLossBatched nn lossFun datas)
          putStrLn $ "Loss after training: " ++  show (evaluateLossBatched trainedNN lossFun datas)
          putStrLn "Some predictions by a trained network:"
          let (pred, grads) = forward trainedNN $ head inputs
          let (pred2, _) = forward trainedNN2 $ head inputs
          print pred 
          putStrLn "pred2"
          print pred2
